...
- **Single-invoice lookup and history** — `get_invoice(invoice_id)` reuses `/invoices/fetch` (the same endpoint `fetch_invoices` uses for listing) with `filter="id={invoice_id}"` and no `data.accounting.sales` constraint, so it works for both sales and purchase invoices without the caller having to know which. There is no dedicated "history" endpoint in the observed traffic (`/invoices/{id}/ksef-preview` turned out to return a rendered HTML preview for PDF generation, not JSON, and `/message-threads/search` is a comment-thread feature that was empty on every invoice tested) — instead, `Invoice.history()` derives a chronological (date, operation) timeline entirely from the `date*` fields already present in the invoice payload returned by `/invoices/fetch` (`dateCreated`, `dateSentToAccounting`, `dateSucceeded`, `dateValidated`, `dateAuthenticated`, `issuedInvoice.sentToKsef.dateSend`, `dateExported`, `dateSentToBuyer`, `dateTransferOrdered`), skipping any field that's absent. The Polish labels are this library's own wording, not scraped from the web UI (which wasn't directly inspected) — they're inferred from the field names and observed values.
